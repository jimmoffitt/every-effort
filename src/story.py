...
Strava's own "Year in Sport" wraps and Instagram/Spotify story recaps —
styled for this app's own orange/flame identity rather than copying any one
brand's exact badge shape or palette.
...
"Download as HTML" export. `data` is the dict from
process_data.wrapped_story_data().
"""
import html as _html

_ORANGE = '#FC4C02'
_ORANGE_LIGHT = '#FCAB87'
_FLAME_1 = '#FC4C02'
_FLAME_2 = '#a12a06'
_BG = '#0b0c0f'
_CARD_BG = '#111319'
    ...
    <section class="slide slide-cover active" data-slide="0">
      <div class="badge-ring">
        <div class="badge-inner">
          ...
          <div class="badge-line badge-line-big">MOTION</div>
        </div>
      </div>
      <div class="cover-year">{data['year']}</div>
      <div class="cover-tag">{data['total_miles']:,.0f} mi &middot; {data['total_hours']:,.0f} hours.
...
def _slide_totals(data):
    rows = [
        ("Total Days Active", f"{data['active_days']:,}"),
        ("Total Hours",       f"{data['total_hours']:,.0f}"),
        ("Total Distance",    f"{data['total_miles']:,.0f} mi"),
        ("Activities",        f"{data['total_activities']:,}"),
    ]
    rows_html = "".join(
...
def _slide_days_active(data):
    ...
    <section class="slide slide-bars" data-slide="2">
      ...
      <div class="big-number">{data['active_days']:,}</div>
...
def _slide_hours(data):
    ...
    # Peak label is an absolutely-positioned HTML span, not SVG <text> — the
    # viewBox is intentionally non-uniform (preserveAspectRatio="none", to
    # fill the card width exactly), and Chrome distorts/mirrors SVG text
    # glyphs under non-uniform scaling. Percent-based CSS position sidesteps
    # that entirely.
    ...
    <section class="slide slide-hours" data-slide="3">
      ...
      <div class="big-number">{data['total_hours']:,.0f}</div>
      <div class="line-chart-wrap">
        <svg class="line-chart" viewBox="0 0 {w} {h}" preserveAspectRatio="none">
          <defs>
            <linearGradient id="areaFill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stop-color="{_ORANGE}" stop-opacity="0.35"/>
              <stop offset="100%" stop-color="{_ORANGE}" stop-opacity="0"/>
            </linearGradient>
          </defs>
          <polygon points="{area}" fill="url(#areaFill)"/>
          <polyline points="{poly}" fill="none" stroke="{_ORANGE}" stroke-width="2.5"
                    stroke-linejoin="round" stroke-linecap="round"/>
          <circle cx="{peak_x:.1f}" cy="{peak_y:.1f}" r="4.5" fill="{_ORANGE_LIGHT}"/>
        </svg>
...
def _slide_streak(data):
    ...
    date_range = (f"{data['streak_start']} &rarr; {data['streak_end']}"
    ...
    <section class="slide slide-streak" data-slide="4">
      ...
      <div class="flame-ring">
        <div class="flame-inner">
          ...
          <div class="flame-unit">weeks</div>
        </div>
      </div>
      <div class="streak-range">{date_range}</div>
      <div class="streak-dots">{dots_html}{more}</div>
    </section>"""


def _slide_top_sports(data):
    sports = data['top_sports']
    max_miles = max((s['miles'] for s in sports), default=1) or 1
    medals = {1: '🥇', 2: '🥈', 3: '🥉'}
    rows_html = "".join(
        f'<div class="rank-row">'
        f'<div class="rank-badge">{medals.get(s["rank"], s["rank"])}</div>'
        f'<div class="rank-icon">{_SPORT_ICONS.get(s["bucket"], "🏅")}</div>'
        f'<div class="rank-body">'
        f'<div class="rank-name">{_esc(s["label"])}</div>'
        f'<div class="rank-bar-track"><div class="rank-bar-fill" '
        f'style="width:{(s["miles"] / max_miles * 100):.1f}%"></div></div>'
        f'</div>'
        f'<div class="rank-value">{s["miles"]:,.0f} mi</div>'
        f'</div>'
        for s in sports
    )
    return f"""
    <section class="slide slide-artists" data-slide="5">
      <div class="eyebrow">TOP SPORTS</div>
      <div class="rank-list">{rows_html}</div>
    </section>"""


_SLIDE_BUILDERS = [_slide_cover, _slide_totals, _slide_days_active,
                   _slide_hours, _slide_streak, _slide_top_sports]
...
.badge-ring {{
  ...
  background: conic-gradient({_ORANGE} 0deg, {_ORANGE_LIGHT} 120deg, {_ORANGE} 240deg, {_ORANGE_LIGHT} 360deg);
  display: flex; align-items: center; justify-content: center;
  margin-bottom: 24px;
  box-shadow: 0 0 40px rgba(252,76,2,0.45);
}}
.badge-inner {{
...
.bar-fill {{ height: 100%; border-radius: 8px; background: {_ORANGE}; }}
.bar-fill.bar-peak {{ background: {_ORANGE_LIGHT}; box-shadow: 0 0 10px rgba(252,171,135,0.6); }}
...
.line-month-peak {{ color: {_ORANGE_LIGHT}; }}
...
.flame-ring {{
  ...
  box-shadow: 0 0 40px rgba(161,42,6,0.5);
}}
.flame-inner {{
  width: 122px; height: 122px; border-radius: 50%; background: {_BG};
  display: flex; flex-direction: column; align-items: center; justify-content: center;
...
.rank-badge {{ width: 22px; font-size: 15px; text-align: center; }}
...
.rank-bar-fill {{ height: 100%; border-radius: 3px; background: {_ORANGE}; }}
.rank-value {{ font-size: 13px; font-weight: 700; color: rgba(255,255,255,0.8); min-width: 56px; text-align: right; }}
...
def render_story_html(data):
    ...
    title = f"Your {data['year']} Year in Motion"
